...
const rupee = (n) => `₹${Number.isInteger(n) ? n : n.toFixed(2)}`;
...
function sortResults(results) {
    ...
    const out = [...results];
    const oosLast = (a, b) => a.stock === b.stock ? 0 : a.stock ? -1 : 1;
    if (mode === "price_asc") {
...
function render() {
    ...
    // Summary line.
    const best = lastResponse.cheapest_price != null && lastResponse.best_platform
...
function buildImage(item) {
    if (!item.image_url)
        return fallbackTile(item.product_name);
